...
pgdg-centos10-10-2.noarch.rpm', quiet=True)
    ...
    def install(self):
        ...
        if not files.exists('/etc/pgbouncer/'):
            run('yum install -y pgbouncer', quiet=True)
            run('systemctl enable pgbouncer')

    def configure(self):
        if files.exists(f'{pg_data}/base'):
            run(f'PATH=$PATH:{pg_bin} PGPASSWORD={pg_password} psql -U postgres -c \"CREATE ROLE pgbouncer \
WITH LOGIN ENCRYPTED PASSWORD \'pgbouncer\';"', quiet=True)
            run(f'PATH=$PATH:{pg_bin} PGPASSWORD={pg_password} createdb -U postgres pgbench', quiet=True)
    ...
    def __get_users(self):
        content = ""
        users = (('pgbouncer', 'pgbouncer'), ('production', 'production'), ('test', 'test'),)
